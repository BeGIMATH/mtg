...
class MTG(PropertyTree):
    ...
    def vertices_iter(self, scale = -1):
        ...
        '''
        if scale < 0:
            return iter(list(self._scale.keys()))
        else:
            return (vid for vid, sid in list(self._scale.items()) if sid == scale)
    ...
    def iter_edges(self, scale=-1):
        ...
        """
        if scale < 0:
            return ((v,k) for k,v in list(self._parent.items()))
        else:
            return ((parent, child) for child, parent in list(self._parent.items()) if self.scale(parent) == scale)
    ...
    def reindex(self, mapping=None, copy=False):
        ...
        if copy:
            ...
            self._parent = dict((mapping[k], mapping.get(v)) for k, v in list(self._parent.items()))
            self._children = dict((mapping[k], [mapping[v] for v in l]) for k, l in list(self._children.items()))
            self._complex = dict((mapping[k], mapping.get(v)) for k, v in list(self._complex.items()))
            self._components = dict((mapping[k], [mapping[v] for v in l]) for k, l in list(self._components.items()))
            self._scale = dict((mapping[k], s) for k, s in list(self._scale.items()))
            for name in self._properties:
                d = self._properties[name]
                self._properties[name] = dict((mapping[k], s) for k, s in list(d.items()))
    ...
    def insert_scale(self, inf_scale=None, partition=None, default_label=None, preserve_order=True):
        ...
        new_scale = dict((vid,sid+1) for vid, sid in list(g._scale.items()) if sid >= inf_scale)
        g._scale.update(new_scale)

        max_id = max(g._scale)
        complex_inf = dict((colors[i], max_id+i+1) for i in range(len(colors)))
        components_sup = dict((vid, complex_inf[cid]) for vid ,cid in list(sup_components.items()))

        # add the new vertices
        g._scale.update( (vid, inf_scale) for vid in list(complex_inf.values()))
        ...
        # And new components
        for vid, component_id in list(components_sup.items()):
            g.add_component(vid,component_id)

        # Add new complex
        for component_id, complex_id in list(complex_inf.items()):
    ...
    def remove_scale(self, scale):
        ...
        new_scale = dict((vid,sid-1) for vid, sid in list(g._scale.items()) if sid > scale)
        g._scale.update(new_scale)
...
def colored_tree(tree, colors):
    ...
    if isinstance(tree, MTG):
        ...
        g._parent.update(dict(((index_scale[k], index_scale[v])
                                for k, v in list(tree._parent.items()) if v is not None and tree.scale(v) == max_scale)))
        for parent, children in list(tree._children.items()):

            if tree.scale(parent) == max_scale:
                g._children[index_scale[parent]] = [index_scale[id] for id in children]
    else:
        g._parent.update(dict(((index_scale[k], index_scale[v])
                                for k, v in list(tree._parent.items()) )))
        for parent, children in list(tree._children.items()):
            g._children[index_scale[parent]] = [index_scale[id] for id in children]

    # Copy the properties of the tree
    for pname, prop in list(tree.properties().items()):
        property = g._properties.setdefault(pname, {})
        for id, v in list(prop.items()):
